...
window.onload = function () {
  ...
  if (document.querySelector('.js_prod__amount-span')) {
    const incrementAndDecrementSpan = document.querySelector(
      '.js_prod__amount-span'
    );
  ...
  dataArray.forEach(product => {
    if (product.type == 'nutra') {
      ...
      if (nutraAmount > product.nutraRange.length) {
        productTotal = Number(
          product.nutraRange[
            (product.quantity - 1) % product.nutraRange.length
          ].toFixed(2)
        );
        if (product.quantity % product.nutraRange.length != 0) {
          nutraAmount =
            nutraAmount - (product.quantity % product.nutraRange.length);
        } else {
      ...
      while (nutraAmount >= product.nutraRange.length && overflow) {
        productTotal += Number(
          product.nutraRange[
            (nutraAmount - 1) % product.nutraRange.length
          ].toFixed(2)
        );
        nutraAmount = nutraAmount - product.nutraRange.length;
      }
...
function removeItem(e) {
  ...
  const updatedCart = getLocalStorageItem().filter(
